...
class UserModel(base):
    ...
    @classmethod
    def return_all(cls,limit, offset, to_dict=True):
...
class PostModel(base):
    ...
    def find_by_titile(cls, title,to_dict=True):
        ...
        if not post:
            return {"messege":f"Post with title {title} not found"}
        else:
            if to_dict:
                return cls.to_dict(post)
            else:
    ...
    @classmethod
    def return_all(cls,limit, offset, to_dict = True):
        posts = session.query(cls).order_by(cls.id).offset(offset).limit(limit).all()
        if to_dict:
            return [cls.to_dict(post) for post in posts]
        else:
            return list(posts)
